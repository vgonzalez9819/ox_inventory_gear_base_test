...
-- Check if a slot is locked for the player
local function isSlotLocked(src, slotCfg)
    if not slotCfg.locked then return false end
    local player = framework.getPlayer(src)
    if not player then return true end
    local jobName, level = '', 0
    if framework.name == 'esx' then
        jobName = player.getJob() and player.getJob().name or ''
        level = player.get('level') or 0
    elseif framework.name == 'qbcore' then
        jobName = player.PlayerData.job and player.PlayerData.job.name or ''
        level = player.PlayerData.metadata and (player.PlayerData.metadata.level or 0) or 0
    end
    if slotCfg.requiredJob and slotCfg.requiredJob ~= jobName then return true end
    if slotCfg.requiredLevel and level < slotCfg.requiredLevel then return true end
    return false
end
...
RegisterNetEvent('gear_system:equip', function(slot, item)
    ...
    if isSlotLocked(src, Config.GearSlots[slot]) then
        TriggerClientEvent('gear_system:slotLocked', src, slot, Config.GearSlots[slot])
        return
    end
