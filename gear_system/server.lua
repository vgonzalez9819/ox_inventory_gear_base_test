...
local function calculateStats(equipped)
    ...
    for slot, itemName in pairs(equipped) do
        ...
        if item and item.stats then
            local rarity = Config.RarityTiers[item.rarity] or {}
            local mult = rarity.statMultiplier or 1.0
            for stat, data in pairs(item.stats) do
                if type(data) == 'table' then
                    local value = (data.value or 0) * mult * (Config.StatScaling[stat] or 1)
...
local function buildClientGear(equipped)
    local result = {}
    for slot, itemName in pairs(equipped) do
        local item = Config.EquipmentItems[itemName]
        if item then
...
local function updatePlayerStats(src)
    ...
    local sendStats = Config.Features.allowStatEffects and stats or {}
    TriggerClientEvent('gear_system:applyStats', src, sendStats)
    TriggerClientEvent('gear_system:setGear', src, buildClientGear(equipped))
    TriggerEvent('gear_system:statsUpdated', src, sendStats)
end

RegisterNetEvent('gear_system:equip', function(slot, itemName)
    local src = source
    local slotCfg = Config.GearSlots[slot]
    local item = Config.EquipmentItems[itemName]
    if not slotCfg or not item or item.slot ~= slot then return end

    if not takeItem(src, itemName) then return end
    ...
    GearData[identifier][slot] = itemName
    saveData()
